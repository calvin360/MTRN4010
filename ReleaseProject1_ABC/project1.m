...
hold on;
h0=[plot(0,0,'rs');plot(0,0,'b');plot(0,0,'-m');plot(0,0,'k+');plot(0,0,'r+')]; %plot pose, lidar, heading,OOI
...
%     pause(dt/30);
    ...
    switch sensorID    %measurement is from?        
        case 1  %  it is a scan from  LiDAR#1!
            ...
            localise(hh,h0,h2,pose,goodOOIx,goodOOIy,xk_G,yk_G);
        continue;      
...
ShowVerification1(data,pathx,pathy,hhx,hhy);
end
%%
...
% process LiDAR scan.  
    ...
    for i=1:size(rr,2)-1
        if (abs(rr(i+1)-rr(i)))>0.9 % if diff is greater than 1m then take a look
            ...
            elseif (0.05<=sqrt((x_cart(temp(2))-x_cart(temp(1)))^2+(y_cart(temp(2))-y_cart(temp(1))^2))<=0.2)
                for k=temp(1):temp(2)
...
%%
function localise(hh,h0,h2,pose,goodOOIx,goodOOIy,xk_G,yk_G)
...
%             hold on
%             th = 0:pi/50:2*pi;
%             xunit = r(i) * cos(th) + goodOOIx(1,i);
%             yunit = r(i) * sin(th) + goodOOIy(1,i);
%             plot(xunit, yunit,'g');
%             hold off
        ...
        y = [r; a];
        %y = f(parameters,x)
        %data is [x_gloabl;y_gloabl]
        %params is [x,y,phi] gloabl
...
%%
function ShowVerification1(data,pathx,pathy,hhx,hhy)
% part c
...
h0=legend({'UGV','lidar scan','heading','COG of poles from lidar','landmarks','Walls (middle planes)','Ground truth (subsampled)','path taken'});
end
